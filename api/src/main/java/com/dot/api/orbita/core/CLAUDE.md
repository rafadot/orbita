...
| `security` | `SecurityConfig` | `/auth/**` e a documentação OpenAPI público, resto autenticado, stateless, **CSRF desabilitado de propósito** (hotspot Sonar S4502 — revisar como Safe no SonarQube, não comentar no código). **CORS habilitado** liberando só a origem de `orbita.frontend-url` (front e API rodam em servidores diferentes, sem domínio/proxy comum — ver [ADR 0010](../../../../../../../../../docs/adr/0010-cors-front-api-servidores-diferentes.md)) — sem `allowCredentials` porque a sessão vai em `Authorization: Bearer`, nunca em cookie. Método **não** declara `throws Exception`: `HttpSecurity.build()` do Spring Security 7.1 não lança mais. |
